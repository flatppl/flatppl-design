...
**Reproducibility.** An engine may compute a density by any method the reductions above admit, stochastic methods included, provided the value is reproducible *with respect to that engine*: the same query, on the same implementation and hardware, yields the same value. A Monte Carlo estimate computed from a fixed seed therefore satisfies this requirement.
