...
| `fn(...)` with `_` holes: anonymous functions | `fn(expr)` wraps a hole expression into an anonymous function. Each `_` inside `fn(...)` is a distinct parameter named `arg1`, `arg2`, ... (left-to-right, normative). Supports both positional and keyword calling. `fn(expr)` lowers to `functionof(...)`. Two-stage lowering: hole abstraction first, then ANF. |
