...
For a categorical over arbitrary values rather than integer indices, such as
[string](03-value-types.md#sec:valuetypes) labels, superpose Diracs at those values:
`ksuperpose(Dirac, p)(value = labels)`. Its variate ranges over `setof(labels)`, and
simplex weights make it normalized (see
...
(see [Betancourt (2012)](17-references.md#betancourt2012)).
